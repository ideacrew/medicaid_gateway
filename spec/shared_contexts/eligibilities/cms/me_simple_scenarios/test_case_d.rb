...
RSpec.shared_context 'cms ME simple_scenarios test_case_d', :shared_context => :metadata do
  ...
  let(:benchmark_premium) do
    { lcsp_premiums: [{ member_identifier: '95', monthly_premium: 430.48 }],
      health_only_slcsp_premiums: [{ member_identifier: '95', monthly_premium: 496.02 }] }
  end
  ...
  let(:medicaid_and_chip) do
    { not_eligible_in_last_90_days: true,
      denied_on: Date.today - 60.days }
  end
  ...
  let(:applicant_reference) do
    { first_name: gerald[:name][:first_name],
      last_name: gerald[:name][:last_name],
      dob: gerald[:demographic][:dob],
      person_hbx_id: gerald[:person_hbx_id] }
  end
