...
RSpec.describe TransferSentReport, dbclean: :after_each do
  before :all do
  ...
  describe "#run" do
    context 'transfers sent from enroll' do
      ...
      before do
        Dir[Rails.root.join("transfer_sent_report_*.csv")].each { |filename| FileUtils.rm_rf(filename) }
        TransferSentReport.run
        report_file = Dir[Rails.root.join("transfer_sent_report_*.csv")].first
      ...
      it 'should create transfer_sent_report' do
        csvs = Dir[Rails.root.join("transfer_sent_report_*.csv")]
      ...
      it 'should return proper values for header IngestionStatus' do
        expect(@report_content['IngestionStatus']).to include(transfer.callback_status)
      end
    end
  end
end
