...
module Aces
  ...
  class GetMecCheck
    ...
    def call(params)
      ...
      @response_payload = yield mec_check(person)
      @response_transmission = yield create_response_transmission(validated_params)
      @response_transaction = yield create_response_transaction(@response_payload)
    end
    ...
    def mec_check(person)
      if person["local_mec_evidence"]
        ...
        return result if result.success?
        if result.failure?
          add_errors({ transmission: @request_transmission, transaction: @request_transaction }, "Mec check call failed", :mec_check)
          status_result = update_status({ transmission: @transmission, transaction: @request_transaction }, :failed, result.failure)
          return status_result if status_result.failure?
          result
        end
      else
        add_errors({ transmission: @request_transmission, transaction: @request_transaction }, "No evedence present", :mec_check)
        status_result = update_status({ transmission: @transmission, transaction: @request_transaction }, :failed, "Person has no evidence")
        return status_result if status_result.failure?
      end
